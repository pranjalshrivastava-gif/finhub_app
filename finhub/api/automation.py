...
def trigger_docusign(docname):
    ...
    try:
        ...
        # Replace with actual DocuSign API call
...
def map_entity_on_save(doc, method):
    """Requirement 1.8: Entity mapping (Country -> Company) – safe version"""
    if not doc.customer:
        return

    # Check if Customer doctype has a 'country' field
    if not frappe.db.has_column("Customer", "country"):
        # Skip mapping without breaking saving
        frappe.log_error("Customer DocType has no 'country' field. Entity mapping skipped.", "Entity Mapping Warning")
        return

    country = frappe.db.get_value("Customer", doc.customer, "country")
    if country:
        # Optional: also check if "Entity Mapping Logic" DocType exists
        if not frappe.db.exists("DocType", "Entity Mapping Logic"):
            return
